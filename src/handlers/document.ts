...
export async function handleDocumentGet(
  ...
  if (!id) return badRequest("Parameter id is missing");

  const user = await config.auth(request);
  if (!user) return unauthorized();

  if (!config.storage.getDocumentsById) return notImplemented();
...
export async function handleDocumentPost(
  ...
  const documents = await config.storage.getDocumentsById(id);
  if (documents.length > 0 && documents[0].userId !== user.id) return forbidden();
...
export async function handleDocumentDelete(
  ...
  if (!id) return badRequest("Parameter id is required.");
  if (!timestamp) return badRequest("Parameter timestamp is required.");
